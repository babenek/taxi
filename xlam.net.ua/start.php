				...
				if($number_second>0){
					while($data_second=mysql_fetch_assoc($result_second)){
						if(!empty($data_second['id'])){ 
							// провер€ем какой столбец б≥л до Їтого нарисован					
							if($tmp==1 || $tmp==2){//если был левый
								print('</td><td valign="top" width="33%" align="center">');	//тогда вставим начало €чейки							
								$tmp++;
								}
							else{
								print('</td></tr><tr><td valign="top" width="34%" align="center">');//был правый или с заголовка - вставл€ем строки
								$tmp=1;								
								}
...
//											' name='.
							...
							print('</table> </td>');// закрыли таблицу, запечатали €чейку	
// END THIRD
							}
						}
					}	
