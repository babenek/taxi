...
//подключение к БД						
	...
	if(isset($_POST['locations'])){//видимо, что-то хотять передать		
		mysql_query('delete from locations;') or die('Query failed: ' . mysql_error());		
		$locations = split(';',$_POST['locations']); 
		foreach($locations as $each){
			if (strlen($each)>10){
				$query='insert into locations(name,id_location,bg,cl) values'.substr($each,0,32).";";	
				mysql_query($query) or die('Query failed: ' . mysql_error());		
				}
			}
		$query='insert into locations(name,id_location,bg,cl) values(\''.date('d.m.y').'\',99,\'FFFFFF\',\'808080\');';	
		mysql_query($query) or die('Query failed: ' . mysql_error());		
		
		$query='insert into locations(name,id_location,bg,cl) values(\''.date('H:i:s').'\',99,\'FFFFFF\',\'808080\');';	
		mysql_query($query) or die('Query failed: ' . mysql_error());		      		
